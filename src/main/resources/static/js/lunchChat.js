...
function wsOpen() {
  ...
  socketVar = new WebSocket(
      "ws://" + location.host + "/lunchChating/" + $("#roomId").val() + "_" + $(
      "#userId").val());
  wsEvt();
}
  ...
  socketVar.onmessage = function (data) {
    ...
    if (msg != null && msg.trim() != '') {
      ...
      if (jsonTemp.type == "getId") {
        ...
        let recommMenu = new Array(jsonTemp.storeList);
        ...
        if (jsonTemp.failReason == 'nameExist') {
          ...
          Swal.fire({
            ...
            text: jsonTemp.failMessage,
            allowOutsideClick: false,
            footer: '<a href="/lunch/room">방 목록으로 이동</a>'
          }, function (isConfirm) {
            if (isConfirm) {
              location.href = '/lunch/room'; //브라우저가 지원하지 않는 경우 페이지 이동처리
            } else {
              location.href = '/lunch/room'; //브라우저가 지원하지 않는 경우 페이지 이동처리
            }
          });
...
function startGame() {
  ...
  commonAjax('/lunch/setRoomStart', roomId, 'post', function () {
    $("#startBtn").remove();
  });
  //showYourJob();
...
function showYourJob() {
  ...
  request.open('GET',"/lunch/startGame/"+$("#roomId").val(),true);
  request.send();
}
...
function voteOpen() {
  ...
  commonAjax('/lunch/getMemberNames', roomId, 'post', function (result) {
...
function mafiaVoteOpen() {
  let roomId = {roomId: $('#roomId').val()};
  commonAjax('/lunch/getCivilNames', roomId, 'post', function (result) {
    let tempJson = JSON.parse(result);
    let tempList = tempJson.memberList;
    let btnHtml = '';
    tempList.forEach(function (e,i,a) {
